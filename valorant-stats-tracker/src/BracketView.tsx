...
function clearDownstream(ids: string[], matches: BracketMatch[]): BracketMatch[] {
  return matches.map(m =>
    ids.includes(m.id)
      ? { ...m, team1: null, team2: null, winner: null, loser: null }
      : m
  );
}
...
function assignColumns(
  ...
  while (queue.length) {
    ...
    for (const nid of [m.winnerNextMatchId, m.loserNextMatchId]) {
      if (!nid) continue;
      const prev = cols.get(nid) ?? -1;
      if (col + 1 > prev) {
        cols.set(nid, col + 1);
        queue.push(nid);
      }
...
function classifySection(
  ...
  for (const m of matches) {
    ...
    if (col === maxCol) sections.set(m.id, "grand-final");
    else if (losersIds.has(m.id)) sections.set(m.id, "losers");
    else sections.set(m.id, "winners");
  }
  return sections;
}
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-sm w-full mx-4 space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-400">
            Moving this team will clear match results in subsequent rounds that
            depend on this slot. This cannot be undone.
          </p>
        </div>
...
function TeamSlot({
  ...
  return (
    <div
      ...
      onDrop={interactive ? (e) => { e.preventDefault(); setDragOver(false); onDrop(ref); } : undefined}
      ...
      } ${interactive && isValidTarget ? "cursor-pointer" : ""} ${
  ...
  for (const m of layout) {
    ...
    if (m.winnerNextMatchId) {
      ...
      if (target) {
        ...
        lines.push(
          <path key={`w-${m.id}`} d={`M${srcX},${srcY} C${mx},${srcY} ${mx},${tgtY} ${tgtX},${tgtY}`}
    ...
    if (m.loserNextMatchId) {
      ...
      if (target) {
        ...
        const mx = (srcX + tgtX) / 2;
        lines.push(
          <path key={`l-${m.id}`} d={`M${srcX},${srcY} C${mx},${srcY} ${mx},${tgtY} ${tgtX},${tgtY}`}
...
export function BracketView({
  ...
  const losersY = hasLosers
    ? Math.min(...layout.filter(m => m.section === "losers").map(m => m.y))
    : null;
