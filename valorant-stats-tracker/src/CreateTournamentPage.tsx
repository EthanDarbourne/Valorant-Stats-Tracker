...
function generateSwissRound(
  teams: Team[],
  existingMatches: TournamentMatchNode[]
): TournamentMatchNode[] {
...
// Step 1 — Format & Setup
function StepFormat({
  format, setFormat, teamCount, setTeamCount, name, setName,
}: {
  format: TournamentTypes; setFormat: (f: TournamentTypes) => void;
  teamCount: number; setTeamCount: (n: number) => void;
...
function StepTeamSelect({
  teamCount, selectedTeams, teamsByRegion, setSelectedTeams,
}: {
  teamCount: number;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="text-sm text-gray-400">
          <span className={`font-bold ${selectedTeams.length === teamCount ? "text-green-400" : "text-red-400"}`}>
...
function StepSeeding({
  selectedTeams, seeds, setSeeds, teamCount, format,
}: {
  selectedTeams: Team[];
  seeds: (Team | null)[];
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-400">
        Drag teams from the pool into seed slots. Seed 1 is the top seed.
        Teams in higher seeds get easier early matchups.
      </p>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Seed Slots */}
        <div>
          <div className="text-xs uppercase tracking-widest text-gray-500 mb-3">Bracket Seeds</div>
...
function GroupSizeSelector({ value, max, onChange }: {
    value: number;
    max: number;
    ...
    return (
        <div className="flex items-center gap-3">
            <button onClick={() => updateSize(value - 1)} disabled={value === 1}>−</button>
            <span>{value}</span>
            <button onClick={() => updateSize(value + 1)} disabled={value === max}>+</button>
        </div>
    );
}
...
function StepGroupSelection({
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-400">
        Drag teams between groups to reassign them
      </p>
...
//         isComplete ? "border-gray-700" : "border-gray-700 hover:border-red-500/50"
//       } bg-gray-900 min-w-[180px]`}
//     >
//       {[match.teamA, match.teamB].map((team, idx) => {
//         const score = idx === 0 ? match.scoreA : match.scoreB;
//         const isWinner = team && winner === team.Id;
//         const isLoser = team && winner !== null && winner !== team.Id;
//         return (
//           <div
//             key={idx}
...
//                 isWinner ? "text-red-400 font-bold" : isLoser ? "text-gray-600" : "text-gray-300"
//               }`}
//             >
//               {team?.Name ?? <span className="text-gray-700 italic">TBD</span>}
//             </span>
//             <span className={`text-sm font-bold ml-2 ${isWinner ? "text-red-400" : "text-gray-500"}`}>
//               {score ?? "-"}
//             </span>
//           </div>
//         );
...
//       {!isComplete && match.teamA && match.teamB && (
//         <div className="px-3 py-1.5 bg-gray-900/80">
//           {editing ? (
...
//                 className="w-10 bg-gray-800 border border-gray-600 rounded text-center text-xs text-white py-0.5 focus:outline-none focus:border-red-500"
//               />
...
//               <button
//                 onClick={submit}
//                 className="ml-1 text-xs bg-red-500 text-white px-2 py-0.5 rounded hover:bg-red-600 transition-colors"
//               >
//                 Set
//               </button>
//               <button
...
//       {isComplete && (
//         <div className="px-3 py-1 bg-gray-900/80 text-center">
...
//         <div key={ri} className="flex flex-col gap-4 min-w-fit">
//           <div className="text-xs uppercase tracking-widest text-gray-500 text-center mb-2">
...
//               <MatchCard key={match.id}  />//match={match} onScore={onScore}
//             ))}
//           </div>
//         </div>
//       ))}
...
//   matches, teams, onScore, onGenerateNextRound,
// }: {
//   matches: TournamentMatchNode[];
//   teams: Team[];
//   onScore: (matchId: string, scoreA: number, scoreB: number) => void;
...
//   const allCurrentComplete = currentRoundMatches.every(m => m.winnerId !== null);
...
//       const loser = m.teamA?.Id === m.winnerId ? m.teamB?.Id : m.teamA?.Id;
//       if (loser !== undefined) losses[loser] = (losses[loser] ?? 0) + 1;
//     }
//   });
//   const standings = [...teams].sort((a, b) => (wins[b.Id] ?? 0) - (wins[a.Id] ?? 0));

//   return (
//     <div className="space-y-8">
//       {/* Standings */}
//       <div>
//         <div className="text-xs uppercase tracking-widest text-gray-500 mb-3">Standings</div>
//         <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
//           {standings.map((team, idx) => (
//             <div key={team.Id} className="flex items-center gap-3 px-3 py-2 bg-gray-900 rounded-lg border border-gray-800">
//               <span className="text-xs text-gray-600 w-5">{idx + 1}.</span>
//               <span className="text-sm font-medium text-gray-200 flex-1">{team.Name}</span>
//               <span className="text-xs font-bold text-green-400">{wins[team.Id] ?? 0}W</span>
//               <span className="text-xs font-bold text-red-400">{losses[team.Id] ?? 0}L</span>
//             </div>
//           ))}
//         </div>
//       </div>
...
//         <div>
//           <div className="text-xs uppercase tracking-widest text-gray-500 mb-3">Round {maxRound}</div>
//           <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
...
//       {allCurrentComplete && maxRound < 5 && (
//         <button
//           onClick={onGenerateNextRound}
//           className="px-6 py-2.5 bg-red-500 text-white rounded-lg font-semibold text-sm hover:bg-red-600 transition-colors"
//         >
//           Generate Round {maxRound + 1}
//         </button>
//       )}
//     </div>
//   );
...
//   matches, teams, numGroups, onScore,
// }: {
//   matches: TournamentMatchNode[];
//   teams: Team[];
//   numGroups: number;
...
//         const groupTeams = teams.slice(gi * groupSize, (gi + 1) * groupSize);
//         const groupMatches = matches.filter(m => m.round === gi + 1);
//         const sorted = [...groupTeams].sort((a, b) => (wins[b.Id] ?? 0) - (wins[a.Id] ?? 0));
...
//                 <div className="text-xs uppercase tracking-widest text-gray-500 mb-2">Standings</div>
//                 {sorted.map((team, idx) => (
//                   <div key={team.Id} className="flex items-center gap-3 px-3 py-2 bg-gray-900 rounded-lg border border-gray-800">
//                     <span className={`text-xs font-bold w-4 ${idx < 2 ? "text-green-400" : "text-gray-600"}`}>{idx + 1}</span>
//                     <span className="text-sm text-gray-200 flex-1">{team.Name}</span>
//                     <span className="text-xs font-bold text-white">{wins[team.Id] ?? 0}W</span>
//                     {idx < 2 && <span className="text-xs text-green-400 font-bold">↑ ADV</span>}
//                   </div>
//                 ))}
//               </div>
//               {/* Matches */}
//               <div className="space-y-2">
//                 <div className="text-xs uppercase tracking-widest text-gray-500 mb-2">Matches</div>
//                 <div className="space-y-2">
//                   {groupMatches.map(m => (
//                     <MatchCard key={m.id} match={m} onScore={onScore} />
//                   ))}
...
function StepBracket({
  ...
  const handleScore = (matchId: string, scoreA: number, scoreB: number) => {
    ...
    //   return { ...m, scoreA, scoreB, winnerId: winner?.Id ?? null };
    // });

    // Advance winner in single elim
    // if (format === "single_elim" || format === "double_elim" || format === "triple_elim") {
    //   const match = updated.find(m => m.id === matchId)!;
    //   if (match.winnerNextMatchId) {
    //     const nextMatch = updated.find(m => m.id === match.winnerNextMatchId);
    //     if (nextMatch) {
    //       const winner = match.scoreA! > match.scoreB! ? match.teamA : match.teamB;
    //       const isFirstFeeder = parseInt(match.id.replace(/\D/g, "").slice(-1)) % 2 === 0;
    //       const updatedNext = {
    //         ...nextMatch,
    //         teamA: isFirstFeeder ? winner : nextMatch.teamA,
    //         teamB: !isFirstFeeder ? winner : nextMatch.teamB,
    //       };
    //       const idx = updated.findIndex(m => m.id === nextMatch.id);
    //       updated[idx] = updatedNext;
    //     }
    //   }
    // }
...
//     const maxRound = matches.length > 0 ? Math.max(...matches.map(m => m.round)) : 0;
//     const newRoundMatches = generateSwissRound(teams, matches, maxRound + 1);
//     setMatches([...matches, ...newRoundMatches]);
//   };

//   if (format === "single_elim" || format === "double_elim" || format === "triple_elim") {
//     return <SingleEliminationBracket matches={matches} onScore={handleScore} />;
//   }
//   if (format === "swiss") {
//     return (
//       <SwissBracket
...
//   }
  if (isStageTournament(format)) {
    return null; // no bracket for these tournaments
    // return <GroupStageBracket matches={matches} teams={teams} numGroups={2} onScore={handleScore} />;
  }
  return null;
}
...
function StepFinalize({
  ...
  return (
    <div className="space-y-8">
      <p className="text-sm text-gray-400">
        Review your tournament setup before creating.
      </p>
        ...
        <section className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {groups.map((group, gi) => (
              <div
                key={gi}
                className="rounded-lg bg-gray-900/40 border border-gray-800 overflow-hidden"
              >
    ...
    <div className="min-h-screen w-screen bg-gray-950 text-white">
      ...
      <div className="border-b border-gray-800 bg-gray-950/80 backdrop-blur sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            ...
            {name && (
              <p className="text-xs text-gray-500 mt-0.5">{name} · {FORMAT_LABELS[format]}</p>
            )}
          </div>
          <button
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <div className="text-xs text-gray-600">Step {step} of {totalStepCount}</div>
